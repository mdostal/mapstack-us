...
describe("hateCrimeDataset (Dataset interface, thirty-third real implementation, real FBI hate crime data)", () => {
  it("conforms to the Dataset interface's basic shape", () => {
    expect(hateCrimeDataset.id).toBe("hate-crime");
    expect(hateCrimeDataset.layers.map((l) => l.id)).toEqual(["hate_crime_rate"]);
    expect(hateCrimeDataset.supportsTime).toBe(false);
  });

  it("returns a 0-100 value with an incident-count detail for a covered city", () => {
