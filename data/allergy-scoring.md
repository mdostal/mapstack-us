...
Plain-language spec for how `allergy-scores.json` is produced. Generated by
`/Users/mdostal/Code/allergy-locator/scripts/gen_spine.py` (re-runnable). Scores
are keyed by the same `id` as the 168-city spine in `cities.json` (the spine
carries no scores and is not modified by scoring).
