 ...
 * by that single column only (pu-2) -- no cross-column combination.
 */
export function ComparisonTable({ selected }: Props) {
  ...
  const sortedCities = sortColumn
    ? sortByValue(cities, (city) => sortColumn.resolved!.dataset.getValue(city.id, sortColumn.resolved!.layer.id)?.value ?? null, direction)
    : cities;
